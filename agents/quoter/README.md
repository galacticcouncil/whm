...
```
GET /relay-fee?chain=ethereum&feeAsset=native         # delivered as native ETH (no FX)
GET /relay-fee?chain=ethereum&feeAsset=0x<erc20>      # delivered as an ERC20 (degraded path)
  → { chain, feeAsset, feeRequested, gasLimit, gasPriceWei, costNativeWei }
```

`feeRequested` = `redeemGas × gasPrice`, converted to `feeAsset` (ERC20 via Hydration's TradeRouter —
...
| `ETH_REDEEM_GAS`      | Representative `redeem` gas limit                       | `150000` |
| `ETH_NATIVE_ASSET_ID` | Tradeable Hydration asset id for WETH (TradeRouter ref) | Required |

> ERC20 → Hydration asset id is resolved from `@galacticcouncil/xc-cfg` route configs.
> `ETH_NATIVE_ASSET_ID` (and ERC20 ids) must be the **TradeRouter-tradeable** registry id — the
