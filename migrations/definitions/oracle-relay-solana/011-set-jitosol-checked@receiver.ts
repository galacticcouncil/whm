...
const step: MigrationStep = {
  name: "011-set-jitosol-checked@receiver",
  description: "Repoint JitoSOL oracle to the CheckedOracle on OracleReceiver",
  action: async (ctx) => {
    const receiverAddress = ctx.outputs["002-deploy-receiver"].proxyAddress;
    const oracle = ctx.env.JITOSOL_CHECKED_ORACLE_ADDRESS;
    const assetId = ctx.env.JITOSOL_ASSET_ID_BYTES32;
    if (!oracle) throw new Error("Missing JITOSOL_CHECKED_ORACLE_ADDRESS");
