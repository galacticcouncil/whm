...
const step: MigrationStep = {
  name: "008-set-wsteth-checked@receiver",
  description: "Repoint WSTETH oracle to the CheckedOracle on OracleReceiver",
  action: async (ctx) => {
    const receiverAddress = ctx.outputs["002-deploy-receiver"].proxyAddress;
    const oracle = ctx.env.WSTETH_CHECKED_ORACLE_ADDRESS;
    const assetId = ctx.env.WSTETH_ASSET_ID;
    if (!oracle) throw new Error("Missing WSTETH_CHECKED_ORACLE_ADDRESS");
    if (!assetId) throw new Error("Missing WSTETH_ASSET_ID");
    if (oracle.toLowerCase() === ZERO) {
      throw new Error(
        "WSTETH_CHECKED_ORACLE_ADDRESS is the zero placeholder — set the deployed CheckedOracle first",
      );
    }
