...
import oracleEmitterIdl from "../../target/idl/oracle_emitter.json";
...
function getConfig() {
  ...
  const programId = optionalArg("--programId") ?? oracleEmitterIdl.address;
...
async function main(): Promise<void> {
  ...
  try {
    const buffers = process.argv.includes("--buffers");
    ...
    if (buffers) {
      ...
      console.log("Closing orphaned deploy buffers");
