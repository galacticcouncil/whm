...
- [Spec](docs/oracle/spec.md) · [Schema](docs/oracle/schema.md)
...
- [Spec](docs/basejump/spec.md) · [Schema](docs/basejump/schema.md) · [Indexer](docs/basejump/indexer.md)
...
- [Spec](docs/intents/spec.md) · [Schema](docs/intents/schema.md) · [Refund](docs/intents/refund.md)
...
contracts/                  # Foundry (Solidity) — @whm/contracts
  src/{basejump,oracles,intents,utils}/
  test/
  scripts/
  foundry.toml

crates/solana/              # Anchor / Cargo workspace — @whm/crates-solana
  programs/oracle-emitter/
    src/
    scripts/
    Anchor.toml
    Cargo.toml
...
migrations/                 # Cross-platform deploy pipelines
  definitions/<merged-migration>/
  actions/
  envs/<context>/
  run.ts

deployments/                # Migration state files
  prod/<migration>.json
  fork/<migration>.json

sh/                         # Cross-cutting bash wrappers
  fork-*.sh
  migrate-*.sh
  verify-*.sh
...
pnpm fork:all           # all four in parallel
...
# Check oracle state on all source chains
pnpm check:oracles
```
...
- **Commit messages**: `scope: description` (lowercase, imperative). Common scopes: `oracle`, `basejump`, `intents`, `bjscan`, `contracts`, `crates`.
