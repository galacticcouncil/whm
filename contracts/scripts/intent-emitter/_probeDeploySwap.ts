 ...
 *   (bridge leg currently disabled in the contract; this validates the SWAP path only).
 *
...
import { checkIfEthereumExecuted, logEvents } from "@whm/chopsticks/events";
...
async function main(): Promise<void> {
  ...
  const hydration = nets.hydration;
  try {
    ...
    const client = new EthClient(hydration, account, { chainId: CHAIN_ID });
    ...
    const swapRes = await client.call(
      proxyAddr,
      encodeFunctionData({
        abi: emitterAbi,
    ...
    await check(hydration, swapRes, "swapAndBridge");

    // ── Did the swap move value? (race-free state reads at the swap block) ──
    const contractAcct = H160.toAccount(proxyAddr);
    const contractWeth = await getTokenBalance(hydration, contractAcct, WETH, swapRes.blockHash);
    const contractGlmr = await getTokenBalance(hydration, contractAcct, GLMR, swapRes.blockHash);
    const deployerDot = await getTokenBalance(hydration, deployerAcct, DOT, swapRes.blockHash);
    console.log(`\n🥢 Post-swap:`);
