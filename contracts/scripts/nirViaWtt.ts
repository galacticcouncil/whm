...
import { isAddress, pad, keccak256, encodeAbiParameters, erc20Abi } from "viem";
 ...
 * Wormhole TokenBridge with a payload (payload-3), addressed to IntentReceiver on Ethereum. This
 * script only bridges — the mrelayer bot picks up the VAA and relays it (redeem → unwrap WETH →
 * native ETH → forward to the OneClick depositAddress), and the nintent agent (agents/nintent)
 * watches the resulting `IntentForwarded` and notifies 1Click. For the no-bot manual relay, see
 * nirRelay.ts.
 *
 ...
 *
 * Returns once the bridge tx is committed; the bot relays and nintent notifies 1Click out-of-band.
 *
 * NOTE: needs a real network and the bot running. The bot quotes marginBps=0 at redeem time; if that
 * exceeds the payload's maxRelayFee it skips as unprofitable and the deposit never lands.
 *
 * Env: RPC, CHAIN_ID (Moonbeam); QUOTER_URL?, MAX_FEE_MARGIN_BPS?
...
async function main(): Promise<void> {
  ...
  // 4. Done. The mrelayer bot relays the payload-3 VAA on Ethereum (redeem → unwrap WETH → native ETH
  //    → forward to depositAddress), and the nintent agent watches IntentForwarded and notifies 1Click.
  //    No need to wait here.
  console.log(
    `\nDone. Bot will relay → nintent notifies 1Click. Track status for ${depositAddress}.`,
  );
}
