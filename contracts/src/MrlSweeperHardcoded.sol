...
contract MrlSweeperHardcoded {
    ...
    /// @param tokens      per-token Moonbeam ERC20 addresses
    /// @param chains      Wormhole recipient chain per token
    /// @param recipients  bytes32 treasury recipient per token (evm left-pad / solana ATA / sui 32b)
    constructor(
        address sa,
        address owner,
        address bridge,
        ...
        require(sa != address(0) && bridge != address(0), "zero addr"); // owner==0 is allowed (disables EOA path)
        SA = sa;
        OWNER = owner;
        BRIDGE = ITokenBridge(bridge);
        for (uint256 i = 0; i < tokens.length; i++) {
            destOf[tokens[i]] = Dest(chains[i], recipients[i], true);
        }
    }
    ...
    /// sweep the full current SA balance of `token` to its hardcoded destination. bal==0 ⇒ no-op.
    function sweep(address token) external auth returns (uint64 seq) {
        return _sweep(token, IERC20(token).balanceOf(SA));
    }

    /// sweep exactly `amount` — lets the caller pace against the Wormhole Governor
    /// (a sub-$100k leg under the daily notional cap, then the big-transfer tail).
    function sweepAmount(address token, uint256 amount) external auth returns (uint64 seq) {
    ...
    function _sweep(address token, uint256 amount) internal returns (uint64 seq) {
        ...
        seq = BRIDGE.transferTokens(token, amount, d.chain, d.recipient, 0, 0);
